...
SELECT '"' + 'LDSB' + '"'					AS BOARDCODE
  ...
  AND a.school_year          = '20182019'
  AND b.take_attendance_flag = 'x'
  AND d.status_indicator_code in ('Active', 'PreReg')
  AND a.school_code in ('SVIEW','ENTPR','TAMWH','SELBY','PRCHL','NWBGH','CTRVL','GRECE','LANOL','CLARC')
  ...
  AND CONVERT(VARCHAR,a.start_date,110)   >= CONVERT(DATETIME,'04-SEP-2018', 110)
  AND CONVERT(VARCHAR,a.end_date,110)    <= CONVERT(DATETIME,'29-JUN-2019', 110)
  ...
  UNION
   ...
   SELECT '"' + 'LDSB' + '"'				AS BOARDCODE
    ...
    , '"' + IsNull((Select top 1
		...
		And (CONVERT(VARCHAR,student_program_class_tracks.start_date,110) >= CONVERT(DATETIME,'01-FEB-2019', 110))
		And (CONVERT(VARCHAR,student_program_class_tracks.end_date,110) <= CONVERT(DATETIME,'30-JUN-2019', 110)
  ...
  AND b.take_attendance_flag = 'x'
  AND d.status_indicator_code in ('Active', 'PreReg')
  AND a.school_code in ('NAPDI','GRECS')
  AND CONVERT(DATETIME,a.start_date, 110)   <= CONVERT(DATETIME, getDate(), 110)
  AND CONVERT(DATETIME,a.end_date, 110)		>= CONVERT(DATETIME, getDate(), 110)
  AND CONVERT(VARCHAR,a.start_date,110)   >= CONVERT(DATETIME,'04-SEP-2018', 110)
  AND CONVERT(VARCHAR,a.end_date,110)    <= CONVERT(DATETIME,'29-JUN-2019', 110)
  ...
  AND CONVERT(VARCHAR,t.start_date,110) >= CONVERT(DATETIME,'04-SEP-2018', 110)
  AND CONVERT(VARCHAR,t.end_date,110) <= CONVERT(DATETIME,'29-JUN-2019', 110)
...
c.school_year = e.school_year
  ...
  AND CONVERT(VARCHAR,c.end_date,110)    <= CONVERT(DATETIME,'29-JUN-2019', 110)
...
AND c.school_year = '20182019'
AND d.person_id not in (select person_id from fs_onsis_teachers 
						where school_year = '20182019' AND CONVERT(DATETIME,effective_date, 110) <= CONVERT(DATETIME, getDate(), 110))
--and
--c.school_code = e.school_code and
--c.school_year = e.school_year
  WHERE
...
WHERE
c.person_id not in (select person_id from fs_onsis_teachers where school_year = '20182019' AND CONVERT(DATETIME,effective_date, 110) <= CONVERT(DATETIME, getDate(), 110))
AND c.school_year = '20182019'
AND c.staff_type_name IN ('Teacher','Teacher LTO', 'ECE', 'Occasional Teach', 'Teacher SPED')
AND CONVERT(DATETIME,c.start_date, 110)   <= CONVERT(DATETIME, getDate(), 110)
AND CONVERT(DATETIME,c.end_date, 110)		>= CONVERT(DATETIME, getDate(), 110)
...
WHERE
c.person_id in (select person_id from fs_onsis_teachers where school_year = '20182019' AND CONVERT(DATETIME,effective_date, 110) <= CONVERT(DATETIME, getDate(), 110))
AND c.person_id not in (select person_id from term_teachers where school_year = '20182019')
AND c.school_year = '20182019'
